...
namespace EFCore.Oracle.Scaffolding.Internal
{
    public class OracleDatabaseModelFactory : IDatabaseModelFactory
    {
        ...
        public OracleDatabaseModelFactory(ILoggerFactory loggerFactory)
        {
        ...
        {
            using (var command = new OracleCommand(GetTablesQuery, _connection))
        ...
        private void GetColumns()
        {
            ...
            {
                using (var command = new OracleCommand(string.Format(GetColumnsQuery, x.Key), _connection))
                using (var reader = command.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        ...
                        var column = new DatabaseColumn
                        {
                            ...
                            DefaultValueSql = reader[4] == DBNull.Value ? null : '\'' + ParseToOracleString(reader[4].ToString()) + '\'',
                            ValueGenerated = valueGenerated
                        };
        ...
        private string ParseToOracleString(string str)
        {
            // Pending the OracleConnector implement OracleCommandBuilder class
            return str
                .Replace("\\", "\\\\")
                .Replace("'", "\\'")
                .Replace("\"", "\\\"");
     ...
     GROUP BY `INDEX_NAME`, `NON_UNIQUE`;";
        ...
        private void GetPrimaryKeys()
        {
            foreach (var x in _tables)
            {
                using (var command = new OracleCommand(string.Format(GetPrimaryQuery, _connection.Database, x.Key.Replace("`", "")), _connection))
                using (var reader = command.ExecuteReader())
                {
                    while (reader.Read())
                    {
     ...
     GROUP BY `INDEX_NAME`, `NON_UNIQUE`;";
        ...
        private void GetIndexes()
        {
            foreach (var x in _tables)
            {
                using (var command = new OracleCommand(string.Format(GetIndexesQuery, _connection.Database, x.Key.Replace("`", "")), _connection))
                using (var reader = command.ExecuteReader())
                {
                    while (reader.Read())
                    {
 ...
 WHERE `TABLE_SCHEMA` = '{0}'
        ...
        private void GetConstraints()
        {
            foreach (var x in _tables)
            {
                using (var command = new OracleCommand(string.Format(GetConstraintsQuery, _connection.Database, x.Key.Replace("`", "")), _connection))
                using (var reader = command.ExecuteReader())
                {
                    while (reader.Read())
                    {
