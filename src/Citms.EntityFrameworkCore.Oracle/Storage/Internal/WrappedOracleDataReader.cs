...
{
    /// <summary>
    /// <see cref="WrappedOracleDataReader"/> wraps <see cref="DbDataReader" /> and enhances <see cref="GetFieldValue{T}"/> to
    /// use reflection to cast from <c>byte[]</c> if the regular method fails; this allows JSON objects to be deserialized.
    /// </summary>
    public class WrappedOracleDataReader : DbDataReader
    {
        private DbDataReader _reader;
        private bool _disposed;

        internal WrappedOracleDataReader(DbDataReader reader)
        {
            _reader = reader;
        }
        ...
        private void CloseReader()
        {
            if (_reader != null)
            {
                // dispose the underlying Oracle data reader
                _reader.Dispose();
                _reader = null;
            }
        }
        ...
        private DbDataReader GetReader()
        {
            if (_reader == null)
            {
                throw new ObjectDisposedException(nameof(WrappedOracleDataReader));
            }
